...
# %% auto 0
__all__ = ['hard_softmax', 'MMD_loss', 'OT_loss', 'UOT_loss', 'density_specified_OT_loss', 'Density_loss', 'Local_density_loss',
...
class UOT_loss(nn.Module):
    ...
    def __init__(self, which='mm_unbalanced', use_cuda=True, reg_m_l2=[5., 5.], detach_M=False, detach_m=False):
        if which not in self._valid:
            raise ValueError(f'{which} not known ({self._valid})')
...
class Density_loss(nn.Module):
    ...
    def __call__(self, source, target, groups = None, to_ignore = None, top_k = 5, source_weights=None, target_weights=None):
        ...
        if groups is not None:
            # for global loss
            c_dist = torch.stack([
                torch.cdist(source[i], target[i]) * source_weights_softmax[i].unsqueeze(-1) * target_weights_softmax[i].unsqueeze(0)
