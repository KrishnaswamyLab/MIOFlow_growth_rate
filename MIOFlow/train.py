...
# %% auto 0
__all__ = ['DEBUG', 'train', 'train_ae', 'training_regimen']

# %% ../nbs/05_train.ipynb 3
import os, sys, json, math, itertools
import pandas as pd, numpy as np
import warnings
from .losses import UOT_loss
...
def train(
    ...
    if isinstance(model, GrowthRateModel) or isinstance(model, GrowthRateSDEModel):
        growth_rate = True
        assert isinstance(criterion, density_specified_OT_loss) or isinstance(criterion, UOT_loss), \
            'Criterion must be density_specified_OT_loss or UOT_loss when using growth_rate=True'
    else:
        growth_rate = False
    ...
    if unbalanced:
        ...
        assert not energy_loss_growth_rate.use_softmax, 'Softmax is not allowed in unbalanced OT loss.'
    ...
    for batch in tqdm(range(n_batches)):
        ...
        if local_loss and not global_loss:
            ...
            for step_idx, (t0, t1) in enumerate(steps):  
                ...
                if use_density_loss: 
                    if growth_rate:
                        if density_detach_m:
                            density_loss = density_fn(data_tp, data_t1, source_weights=m_tp.detach(), top_k=top_k)
                        else:
                            density_loss = density_fn(data_tp, data_t1, source_weights=m_tp, top_k=top_k)
                        if DEBUG and m_tp.min() <= 0.:
                ...
                if growth_rate and lambda_unif_velo > 0:
                    unif_velo_loss = unif_velo_loss_growth_rate(model.func, data_tp, m_tp, time[-1])
                    ...
                    unif_velo_loss = unif_velo_loss(model.func, data_tp, time[-1])
                    loss += lambda_unif_velo * unif_velo_loss
                    all_losses['unif_velo_loss'].append(unif_velo_loss.item())
        ...
        # apply global loss
            ...
            if use_density_loss:                
                if growth_rate:
                    if density_detach_m:
                        density_loss = density_fn(data_tp, data_ti, groups, to_ignore, top_k, source_weights=m_tp.detach())
                    else:
                        density_loss = density_fn(data_tp, data_ti, groups, to_ignore, top_k, source_weights=m_tp)
                else:               
            ...
            if growth_rate and lambda_unif_velo > 0:
                unif_velo_loss = unif_velo_loss_growth_rate_seq(model.func, data_tp[non_ignore_idx,...], m_tp[non_ignore_idx,...], time[non_ignore_idx,...])
                loss += lambda_unif_velo * unif_velo_loss
                if DEBUG:   
                    print("Unif velo loss", unif_velo_loss.item())
            elif lambda_unif_velo > 0:
                unif_velo_loss = unif_velo_loss_seq(model.func, data_tp[non_ignore_idx,...], time[non_ignore_idx,...])
                loss += lambda_unif_velo * unif_velo_loss
    ...
    return local_losses, batch_losses, globe_losses, all_losses, plans
...
def training_regimen(
    ...
    for epoch in tqdm(range(n_local_epochs), desc='Pretraining Epoch'):
        ...
        train_result = train(
            ...
            lambda_ot=lambda_ot, lambda_marginal=lambda_marginal,
        ...
        l_loss, b_loss, g_loss, all_losses, plans = train_result
    ...
    if return_plan:
        return local_losses, batch_losses, globe_losses, losses_dict, plans
    else:
